...
from models import ConvNet, CConvNet, S2ConvNet, MLP
...
if __name__ == '__main__':
    ...
    parser.add_argument('--overshoot', type=float, default=0.01)
    ...
    elif df[df['tags.mlflow.runName']==str(args.run_name)]['tags.model'].values[0] == 'MLP':
        model = MLP(hparams, None, None).eval()
    elif df[df['tags.mlflow.runName']==str(args.run_name)]['tags.model'].values[0] == 'S2ConvNet':
        model = S2ConvNet(hparams, None, None).eval()
    else:
        raise NotImplementedError(f"Model has to be 'ConvNet', 'CConvNet', 'S2ConvNet' or 'MLP'. Got {df[df['tags.mlflow.runName']==str(args.run_name)]['tags.model'].values[0]}.")
    ...
    attack = LinfDeepFoolAttack(steps=steps, candidates=candidates, overshoot=overshoot, loss=loss)
    ...
    save_pickle(save_path, 'params.pickle', {'epsilons': epsilons, 'steps': steps, 'candidates': candidates, 'overshoot': overshoot, 'loss': loss})
