...
class CConv2d(torch.nn.Module):
    ...
    def forward(self, input:Tensor)->Tensor:
        x_pad = torch.nn.functional.pad(input, self.padding, mode='circular')
        x_conv = self.conv(x_pad)
        return x_conv

    
    
class CConvNet(pl.LightningModule):
    def __init__(self, hparams, train_data, test_data):
        super().__init__()
        ...
        for i in range(len(self.channels)-1):
            in_ch = self.channels[i]
            out_ch = self.channels[i+1]
            module_list.append(CConv2d(in_ch, out_ch, kernel_size=self.kernels[i], stride=self.strides[i]))
            if self.activation_fn == 'ReLU':
                module_list.append(torch.nn.ReLU())
            elif self.activation_fn == 'LeakyReLU':
                module_list.append(torch.nn.LeakyReLU())
