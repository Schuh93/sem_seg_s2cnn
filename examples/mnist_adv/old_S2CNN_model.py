...
class S2ConvNet(pl.LightningModule):
    def __init__(self, hparams, train_data, test_data):
        ...
        if isinstance(self.kernel_max_beta, float):
            self.kernel_max_beta = [self.kernel_max_beta] * (len(self.channels) + 1)

        assert len(self.channels) == len(self.bandlimit) == len(self.kernel_max_beta) - 1
        possible_activation_fns = ['ReLU', 'LeakyReLU']
        assert self.activation_fn in possible_activation_fns
        
        
        grid_s2 = s2_near_identity_grid(max_beta=self.kernel_max_beta[0] * np.pi)
        grids_so3 = [
            so3_near_identity_grid(max_beta=max_beta * np.pi) for max_beta in self.kernel_max_beta[1:]
        ]
